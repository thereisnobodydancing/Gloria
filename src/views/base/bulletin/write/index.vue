    ...
    <n-spin :show="showLoading">
      ...
      >
        <div class="bg-white w-11/12 rounded p-8 mx-auto relative">
          ...
          <!-- 4-submit -->
          <div class="mt-2 px-2">
            <n-button type="primary" @click="submit">{{ btnObj[route.query.type] }}</n-button>
          </div>
        </div>
      </div>
    </n-spin>
...
const showLoading = ref(route.query.type === 'edit' ? true : false)
// 编辑器相关
...
const submit = function() {
  if(!data.type || !data.title || !data.contents || !data.recipientDTOList || data.recipientDTOList.length === 0) {
    message.error('输入信息不完整')
    return
  }
  dialog.warning({
    title: '提示',
    content: `你确定要${btnObj[route.query.type]}这篇公告吗？`,
    positiveText: '确定',
    negativeText: '不确定',
    onPositiveClick: () => {
      ...
      if(route.query.type === 'create') createBulletin()
      if(route.query.type === 'edit') editBulletin()
    },
    onNegativeClick: () => {}
  })
}
...
// 创建公告
const createBulletin = function() {
  api.post('/announcement/submit', data).then((res) => {
    if(res.data.code === 20000) {
