    ...
    <div class="w-full space-y-6">
      ...
      >
        <div class="flex items-center">
          <p class="font-bold text-sm">物品明细 {{ index+1 }}</p>
          <button 
            v-if="index > 0"
        ...
        <div class="ml-2 space-y-2">
          ...
          <div class="w-full flex items-center">
            ...
            <n-input v-model:value="item.name" placeholder="请输入物品名称" clearable />
          </div>
          
          <!-- 物品数量 -->
          <div class="w-full flex items-center">
            <p class="text-sm text-gray-400 w-20 flex-shrink-0 leading-8">物品数量：</p>
            <n-input-number v-model:value="item.num" placeholder="请输入物品数量" clearable class="w-full" />
          </div>
        </div>
      </div>
      <div class="px-20">
        <n-button type="tertiary" @click="addItem">+增加物品明细</n-button>
        <p v-if="props.options.desc" class="mt-2 text-xs text-gray-400">{{ props.options.desc }}</p>
      </div>
    </div>
  </div>
</template>
...
const addItem = function() {
  form.value[props.options.id].push({ name: '',  num: null })
}
// 删除明细
