...
<template>
  <div 
    v-if="form[props.options.id].expenseList && form[props.options.id].expenseList.length > 0" 
    class="w-full"
  >
    <div class="w-full space-y-2.5">
      <div
        v-for="(item, index) in form[props.options.id].expenseList" 
      ...
      <p v-if="form[props.options.id].total" class="mt-1 text-sm text-gray-500 space-x-10">
        <span>开销总计：{{ form[props.options.id].total }}</span>
        <span>大写金额：{{ numToCny(form[props.options.id].total) }}</span>
      </p>
    </div>
...
const getTotalExpenses = function() {
  let sum = form.value[props.options.id].expenseList.reduce((sum, item) => +sum + +item.amountSpent, 0)
  form.value[props.options.id].total = sum + ''
}

// 增加明细
const addExpenlist = function() {
  form.value[props.options.id].expenseList.push({ expenseUse: '',  amountSpent: '' })
}
// 删除明细
const deleteExpenlist = function(price, index) {
  if(index > 0) {
    form.value[props.options.id].expenseList.splice(index, 1)
